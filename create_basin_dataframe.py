...
f_mass = "F:/HMA_glacio_hydro_index/glac_mass_monthly/all_rgi_glac_mass_monthly_ensemble_ssp245.nc"
f_runoff = "F:/HMA_glacio_hydro_index/glac_runoff_monthly/all_rgi_glac_runoff_monthly_ensemble_ssp245.nc"
f_sh = "F:/HMA_impact_index/level_7_basins/level_7_selected/RGI_13_14_15_selected_basins.shp"
f_pr = "F:/HMA_glacio_hydro_index/precipitation/pr_month_downscaled_AWI-CM-1-1-MR_scenarioSSP2-45_20150101-21001231.nc"

# Open the basin shapefil as a GeoDataFrame
bas= gpd.read_file(f_sh)

# Open the datasets as a dask array with the chunks being each model and 12 months (a year)
glac_chunk_dict = {'model':1,'time':12}
pr_chunk_dict = {'time':12}
ds_mass = xr.open_mfdataset(f_mass, chunks=glac_chunk_dict)
ds_runoff = xr.open_mfdataset(f_runoff, chunks=glac_chunk_dict)
pr = xr.open_mfdataset(f_pr, chunks= pr_chunk_dict)
...
mass_df = gpd.GeoDataFrame(data=ds_mass.glac_mass_monthly.isel(model= 0, glacier=slice(None), time=slice(180,1212)),geometry=pts,columns=ds_mass.time.isel(time =slice(180,1212)), crs='EPSG:4326')
runoff_df = gpd.GeoDataFrame(data=ds_runoff.glac_runoff_monthly.isel(model= 0, glacier=slice(None), time=slice(180,1212)),geometry=pts,columns=ds_mass.time.isel(time =slice(180,1212)), crs='EPSG:4326')
...
encoded.to_netcdf("F:/HMA_glacio_hydro_index/gindex_AWI-CM-1-1-MR_scenarioSSP2-45.nc", mode="w")
